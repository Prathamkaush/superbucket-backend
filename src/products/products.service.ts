...
function mapVariantInput(variant: any, index: number) {
  ...
  return {
    ...
    isDefault: index === 0 ? variant.isDefault !== false : parseBool(variant.isDefault),
  };
}

function mapNutritionFactInput(fact: any, index: number) {
  if (!fact.name || !fact.amount) {
    throw new BadRequestException(`Nutrition fact ${index + 1} needs name and amount`);
  }
...
async create(body: any, files: any) {
  ...
  const gstRate = body.gstRate !== undefined && body.gstRate !== "" ? Number(body.gstRate) : 0;
  ...
  if (isNaN(weight) || weight < 0.05 || weight > 10) {
    throw new BadRequestException("Product weight must be between 0.05kg and 10kg");
  }

  // -------------------- SIZES --------------------
  const sizes = parseJsonArray(body.sizes, "sizes") as { size: string; stock: number }[];
  const variants = parseJsonArray(body.variants, "variants").map(mapVariantInput);
  const nutritionFacts = parseJsonArray(body.nutritionFacts, "nutritionFacts").map(mapNutritionFactInput);

  if (!Array.isArray(sizes) || (sizes.length === 0 && variants.length === 0)) {
    throw new BadRequestException("At least one variant or legacy size is required");
  }
  ...
  const stockFromVariants = variants.reduce((sum, v) => sum + Number(v.stock), 0);
  const stockFromSizes = sizes.reduce((sum, s) => sum + Number(s.stock || 0), 0);
...
return this.prisma.$transaction(async (tx) => {
  const product = await tx.product.create({
    data: {
      ...
      servingsPerContainer: body.servingsPerContainer ? Number(body.servingsPerContainer) : null,
      ...
      certifications: parseJsonValue(body.certifications, "certifications") ?? [],
      ...
      type: { connect: { id: Number(body.typeId) } },
      subtype: { connect: { id: Number(body.subtypeId) } },
...
async update(id: number, body: any, files: any) {
  ...
  if (body.discountValue !== undefined) {
    const discountValue = body.discountValue !== "" ? Number(body.discountValue) : null;
    data.discountValue = discountValue !== null ? new Prisma.Decimal(discountValue) : null;
  }
  ...
  if (body.servingsPerContainer !== undefined) {
    data.servingsPerContainer = body.servingsPerContainer ? Number(body.servingsPerContainer) : null;
  }
  ...
  for (const field of supplementDecimalFields) {
    if (body[field] !== undefined) data[field] = parseDecimalOrNull(body[field]);
  }
  ...
  if (body.certifications !== undefined) {
    data.certifications = parseJsonValue(body.certifications, "certifications") ?? [];
  }
  ...
  if (body.subtypeId) {
    const subtypeId = Number(body.subtypeId);
    if (!isNaN(subtypeId)) {
      data.subtype = { connect: { id: subtypeId } };
    }
  }
    ...
    if (body.variants !== undefined) {
      ...
      if (variants.length) {
        ...
        await tx.product.update({
          where: { id },
          data: {
            stock: variants.reduce((sum, variant) => sum + Number(variant.stock), 0),
          },
        });
      }
    }
...
async getHomeDiscounts(limit = 8) {
 ...
 // ----------------------------------
  ...
  async bulkUploadWithImages(
    ...
    results.imageStats.unused = Array.from(imageMap.keys())
      .filter((img) => !usedImages.has(img)) as string[];
  ...
  async validateBulkUploadWithImages(
    ...
    validationResults.imageStats.unreferenced = Array.from(imageMap.keys())
